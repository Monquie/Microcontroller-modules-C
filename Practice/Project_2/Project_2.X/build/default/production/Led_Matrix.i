...
volatile unsigned char row __attribute__((address(0xF8A)));
volatile unsigned char row_io __attribute__((address(0xF93)));
volatile unsigned char gcol __attribute__((address(0xF8B)));
volatile unsigned char gcol_io __attribute__((address(0xF94)));
volatile unsigned char rcol __attribute__((address(0xF8C)));
volatile unsigned char rcol_io __attribute__((address(0xF95)));
# 19 "Led_Matrix.c"
void init()
{
...
void rom2ram()
{
    ...
    for(i=0; i<8; i++)
    {
        rambuf[0][i] = maurom[0][i];
        rambuf[1][i] = maurom[1][i];
    }
}
...
{
    row = 0;
    gcol=rambuf[1][rowidx];
    rcol=rambuf[0][rowidx];
    row=1<<rowidx++;
    rowidx%=8;
}
void main()
